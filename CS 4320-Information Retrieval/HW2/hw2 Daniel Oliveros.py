...
COLLABORATORS = [ 
    ('Garrett Bogart', 'Coerced me into actually doing this assignment on time')
    ]
...
class BetterIndex(object):
    ...
    def getSet(self, text):
        word = self._rotate(text)
        output = crawl_tree(self._bt.root, word)
        return output
    ...
    def wildcard_search_or(self, text):
        words = self.tokenize(text,True)
        setA = set()
        for term in words:
            setA = setA.union(self.getSet(term))
        output = ""
        for term in setA:
            output += str(term)
        if (len(setA)==0):
